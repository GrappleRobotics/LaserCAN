/* For some reason, link.x in cortex-m-rt fails to insert the .metadata section,
   so we use our own copy. It's functionally the same, but for some reason this works
   and it doesnt. Link scripts may as well be black magic at this point. */

MEMORY
{
  FLASH : ORIGIN = 0x08005000, LENGTH = 44K
  /* 0x2000_0000 - 0x2000_0100 is reserved for the bootloader */
  RAM : ORIGIN = 0x20000100, LENGTH = 20K-256
}
...
SECTIONS
{
  ...
  .vector_table ORIGIN(FLASH) :
  {
   ...
   .metadata (ORIGIN(FLASH) + 0x150) : {
      KEEP(*(.metadata.magic));
      KEEP(*(.metadata.version));
      KEEP(*(.metadata.model_id));
   } > FLASH

   .firmware_flag (ORIGIN(FLASH) + 0x190) : {
      LONG(0x1234ABCD); /* Reload Firmware Flag. 0xFFFF_FFFF = Reload Firmware, anything else = OK */
   } > FLASH
